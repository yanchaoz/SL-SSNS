...
def post_lmc(affs):
    ...
    for z in range(watershed.shape[0]):
        wsz, max_id = ws.distance_transform_watershed(boundary_input[z], threshold=.25, sigma_seeds=2.0)
        wsz += offset
        offset += max_id
        watershed[z] = wsz
    ...
    costs = mc.transform_probabilities_to_costs(costs, edge_sizes=edge_sizes, beta=0.35)
