...
def conv2dBlock(in_planes, out_planes, kernel_size=[(3, 3)], stride=[1], padding=[0], bias=[True], pad_mode=['zero'],
                bn_mode=[''], relu_mode=[''], init_mode='kaiming_normal', bn_momentum=0.1, dilation_size=None):
    # easy to make VGG style layers
    layers = []
    if dilation_size is None:
    ...
    for i in range(len(in_planes)):
        if in_planes[i] > 0:
            layers += getConv2d(in_planes[i], out_planes[i], kernel_size[i], stride[i], padding[i], bias[i],
                                pad_mode[i], init_mode, dilation_size[i])
        if bn_mode[i] != '':
...
def merge_add(x1, x2):
    # x1 bigger
    offset = [(x1.size()[x] - x2.size()[x]) // 2 for x in range(2, x1.dim())]
    return x2 + x1[:, :, offset[0]:offset[0] + x2.size(2), offset[1]:offset[1] + x2.size(3),
                offset[2]:offset[2] + x2.size(4)]
